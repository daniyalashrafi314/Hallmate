...
const StaffForum: React.FC = () => {
  ...
  const handlePostLike = async (postIdToToggle: number) => {
    ...
    try {
      ...
      setPosts((prev) => prev.map((post) => (
        post.post_id === postIdToToggle
          ? { ...post, viewer_has_liked: liked, like_count: likeCount }
          : post
      )));
      setSelectedPost((prev) => (
        prev && prev.post_id === postIdToToggle
          ? { ...prev, viewer_has_liked: liked, like_count: likeCount }
          : prev
      ));
    } catch (err) {
      setPosts((prev) => prev.map((post) => (
        post.post_id === postIdToToggle
          ? { ...post, viewer_has_liked: previousLiked, like_count: previousCount }
          : post
      )));
      setSelectedPost((prev) => (
        prev && prev.post_id === postIdToToggle
          ? { ...prev, viewer_has_liked: previousLiked, like_count: previousCount }
          : prev
      ));
  ...
  const submitPost = async (event: React.FormEvent) => {
    ...
    try {
      ...
      const response = await fetch(
        isEdit
          ? `${API_BASE_URL}/forum/posts/${editingPost!.post_id}`
  ...
  useEffect(() => {
    loadPosts(1, false);
    // New searches and tabs should reset the list and fetch from page one.
          ...
          <div className="max-w-2xl space-y-3">
            ...
            <div>
              ...
              <p className="mt-2 max-w-2xl text-sm leading-6 text-slate-600">
                Read hall updates, post announcements, and discuss with staff in your hall community.
              </p>
            </div>
          </div>
          ...
          <div className="mt-4 rounded-2xl border border-slate-200 bg-white p-4">
            <div className="flex flex-wrap items-center justify-between gap-3">
              <div>
                ...
                <p className="mt-1 text-sm text-slate-600">Filter posts by the forum categories.</p>
              </div>
                ...
                {selectedPost.genres && selectedPost.genres.length > 0 && (
                  <div className="mt-5 flex flex-wrap gap-2">
                    {selectedPost.genres.map((genre) => (
                      <span key={genre} className="rounded-full border border-slate-200 bg-slate-50 px-3 py-1 text-sm text-slate-600">
                        {genre}
                      </span>
                    ))}
                  </div>
              ...
              <section className="rounded-[1.75rem] border border-slate-200 bg-white p-6 shadow-sm">
                ...
                <div className="mt-6">
                  {visibleComments.length === 0 ? (
                    ...
                    <div className="space-y-3">
                      {visibleComments.map((comment) => renderCommentTree(comment))}
                    </div>
                  )}
                </div>
              </section>
            ...
            <div className="rounded-[1.75rem] border border-dashed border-slate-200 bg-white p-10 text-center shadow-sm">
              ...
              <p className="mt-2 text-sm leading-6 text-slate-500">
                {search || selectedGenres.length > 0
                  ...
                  : 'There are no forum posts in this tab yet.'}
              </p>
            </div>
          ) : (
                  ...
                  <article key={post.post_id} className="group rounded-[1.75rem] border border-slate-200 bg-white p-6 shadow-sm transition hover:-translate-y-0.5 hover:shadow-md">
                    <div className="flex flex-col gap-5 lg:flex-row lg:items-start lg:justify-between">
                      ...
                      >
                        ...
                        <h2 className="mt-3 text-2xl font-bold text-slate-900 transition group-hover:text-sky-700">
                          {post.title}
                        </h2>
                        ...
                        {post.genres && post.genres.length > 0 && (
                          <div className="mt-4 flex flex-wrap gap-2">
                            {post.genres.map((genre) => (
                              <span key={genre} className="rounded-full border border-slate-200 bg-slate-50 px-3 py-1 text-xs font-medium text-slate-600">
                                {genre}
                              </span>
                            ))}
                          </div>
          ...
          <div className="max-h-[90vh] w-full max-w-3xl overflow-y-auto rounded-[2rem] bg-white shadow-2xl">
            <div className="flex items-start justify-between gap-4 border-b border-slate-200 px-6 py-5">
              <div>
                ...
                <p className="mt-1 text-sm text-slate-500">Share a hall update, announcement, or discussion topic.</p>
              </div>
              <button
                type="button"
            ...
            <form onSubmit={submitPost} className="space-y-5 px-6 py-6">
              ...
              <div className="flex flex-wrap items-center gap-3">
                ...
                <span className="text-sm text-slate-500">
                  Public posts are visible across halls. Hall-only posts stay within your hall.
                </span>
              </div>
          ...
          <div className="w-full max-w-2xl rounded-[2rem] bg-white shadow-2xl">
            <div className="flex items-start justify-between gap-4 border-b border-slate-200 px-6 py-5">
              <div>
                ...
                <p className="mt-1 text-sm text-slate-500">Forum author details</p>
              </div>
              <button
                type="button"
            ...
            <div className="grid gap-6 px-6 py-6 md:grid-cols-[160px_1fr]">
              <div className="flex justify-center md:block">
                <div className="flex h-40 w-40 items-center justify-center overflow-hidden rounded-[1.75rem] border border-slate-200 bg-slate-50">
                  {profileLoading ? (
                    <Loader2 className="w-8 h-8 animate-spin text-slate-400" />
                  ) : profilePhotoUrl ? (
                    <img src={profilePhotoUrl} alt={selectedProfile.display_name} className="h-full w-full object-cover" />
                  ) : (
              ...
              <div className="space-y-4">
                {profileLoading ? (
                  ...
                  <>
                    <div>
                      <h4 className="text-3xl font-bold text-slate-900">{selectedProfile.display_name}</h4>
                      <p className="mt-1 text-sm text-slate-500">{selectedProfile.user_type}</p>
                    </div>

                    <div className="grid gap-3 sm:grid-cols-2">
                      <ProfileItem label="Entity ID" value={selectedProfile.entity_id} />
                      <ProfileItem label="Hall" value={selectedProfile.hall_name || 'Not available'} />
                      <ProfileItem label="Room" value={selectedProfile.room_id !== null ? String(selectedProfile.room_id) : 'Not available'} />
                      <ProfileItem
                        label="Role / Status"
                        value={selectedProfile.user_type === 'student' ? selectedProfile.student_status || 'Not available' : selectedProfile.staff_role || 'Not available'}
                      />
                      <ProfileItem label="User Type" value={selectedProfile.user_type} />
                      <ProfileItem label="Photo" value={selectedProfile.has_photo ? 'Available' : 'Not available'} />
                    </div>

                    {selectedProfile.phone_number && (
                      <ProfileItem label="Phone" value={selectedProfile.phone_number} />
                    )}
                  </>
                )}
              </div>
